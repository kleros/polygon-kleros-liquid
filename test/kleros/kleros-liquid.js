...
const MockChainlinkRNG = artifacts.require('./contracts/mocks/MockChainlinkRNG.sol')
const KlerosLiquid = artifacts.require('./contracts/kleros/KlerosLiquid.sol')
const BridgedPinakion = artifacts.require('./contracts/mocks/BridgedPinakionMock.sol')
...
contract('KlerosLiquid', (accounts) => {
  ...
  beforeEach(async () => {
    ...
    bridgedPinakion = await deployProxy(BridgedPinakion, [
      governor, // receiver
    ...
    pinakion = await deployProxy(wPinakion, [
      ...
      'WPNK', // _tokenSymbol
      bridgedPinakion.address // BridgedPinakion
    ])
    randomNumber = 10
    RNG = await MockChainlinkRNG.new(randomNumber, governor)
    minStakingTime = 1
    maxDrawingTime = 1
    const {
    ...
    await RNG.changeKlerosLiquid(klerosLiquid.address, {
      from: governor,
    })
  })
  ...
  it('Should implement the spec, https://docs.google.com/document/d/17aqJ0LTLJrQNSk07Cwop4JVRmicaCLi1I4UfYeSw96Y.', async () => {
    ...
    const lastRequestId = await RNG.lastRequestId()
    await RNG.rawFulfillRandomness(lastRequestId)
    for (const dispute of disputes)
      await klerosLiquid.setStake(
        dispute.subcourtID,
        subcourtMap[dispute.subcourtID].minStake
  ...
  it('Should change the request ID when changing the `RNGenerator` during the generating phase.', async () => {
    ...
    const newRequestId = await RNG.lastRequestId()
    expect(web3.utils.toBN(lastRequestId).eq(web3.utils.toBN(newRequestId))).to.equal(false)
  })
