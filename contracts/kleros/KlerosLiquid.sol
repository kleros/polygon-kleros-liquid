 ...
 *  @authors: [@fnanni-0, @MerlinEgalite]
 *  @reviewers: []
 *  @auditors: []
 *  @bounties: []
 *  @deployments: []
 ...
 */
contract KlerosLiquid is Initializable, TokenController, Arbitrator {
    /* Enums */
    ...
    IChainlinkRNG public RNGenerator; // The random number generator contract.
    // General Dynamic
    ...
    bytes32 public rngRequestId; // Chainlink's request id for the next random number.
     ...
     */
    function changeRNGenerator(IChainlinkRNG _RNGenerator) external onlyByGovernor {
        RNGenerator = _RNGenerator;
        if (phase == Phase.generating) {
            rngRequestId = RNGenerator.requestRN(0);
        }
    }
    ...
    function passPhase() external {
        if (phase == Phase.staking) {
            ...
            RN = RNGenerator.getRN(rngRequestId);
